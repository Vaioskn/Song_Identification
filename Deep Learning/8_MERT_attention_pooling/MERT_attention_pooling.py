...
DRIVE_OUTPUT_DIR = './MERT_Attention_Test_Results'
os.makedirs(DRIVE_OUTPUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_mert_attention_model.pt')
LABELMAP_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_mert_attention_label_map.pkl')
...
class WaveformDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs1 = self.feature_extractor(y1, sampling_rate=SR, return_tensors="pt", padding=True)
        inputs2 = self.feature_extractor(y2, sampling_rate=SR, return_tensors="pt", padding=True)
...
def train_and_evaluate():
    ...
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, num_workers=0, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=True)

    model = MERTForSongID(model_name=MODEL_NAME, emb_dim=EMB_DIM, n_classes=len(id2idx)).to(device)
    ...
    print(f"Starting MERT Fine-Tuning with Attention Pooling for {EPOCHS} epochs...")
    for epoch in range(1, EPOCHS + 1):
        ...
        for y1, y2, labels in train_pbar:
            ...
            train_pbar.set_postfix(loss=f"{loss.item():.4f}", temp=f"{model.temperature.item():.3f}")
...
def evaluate_final_model(model, feature_extractor, query_track_ids, id2idx, n_clips_ref=5):
    ...
    with torch.no_grad():
        for tid in tqdm(query_track_ids, desc="Building DB"):
            ...
            for _ in range(n_clips_ref):
                ...
                inputs = feature_extractor(y_clip, sampling_rate=SR, return_tensors="pt", padding=True).to(device)
    ...
    with torch.no_grad():
        for query_tid in tqdm(query_track_ids, desc="Querying"):
            ...
            inputs_query = feature_extractor(y_query, sampling_rate=SR, return_tensors="pt", padding=True).to(device)
