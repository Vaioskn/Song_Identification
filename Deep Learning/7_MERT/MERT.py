...
DRIVE_OUTPUT_DIR = 'Audio_Project_MERT_Results'
os.makedirs(DRIVE_OUTPUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_mert_model.pt')
LABELMAP_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_mert_label_map.pkl')
...
class MERTForSongID(nn.Module):
    ...
    def forward(self, x):
        ...
        mert_embedding = torch.mean(mert_out, dim=1)
...
def train_and_evaluate():
    ...
    print(f"Starting MERT fine-tuning for {EPOCHS} epochs...")
...
def evaluate_final_model(model, feature_extractor, query_track_ids, id2idx, n_clips_ref=5):
    ...
    print("\n" + "="*50)
    print(f"Final Top-1 Accuracy (MERT Fine-tuned): {accuracy:.2%}")
    print("="*50)
