...
DRIVE_OUTPUT_DIR = 'AST_Results'
os.makedirs(DRIVE_OUTPUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_ast_model.pt')
LABELMAP_PATH = os.path.join(DRIVE_OUTPUT_DIR, 'finetuned_ast_label_map.pkl')
...
def train_and_evaluate():
    ...
    model = ASTForSongID(model_name=MODEL_NAME, emb_dim=EMB_DIM, n_classes=len(id2idx)).to(device)
    ...
    print(f"Starting AST fine-tuning for {EPOCHS} epochs...")
...
def evaluate_final_model(model, feature_extractor, query_track_ids, id2idx, n_clips_ref=5):
    ...
    print("\n" + "="*50)
    print(f"Final Top-1 Accuracy (AST Fine-tuned): {accuracy:.2%}")
    print("="*50)
