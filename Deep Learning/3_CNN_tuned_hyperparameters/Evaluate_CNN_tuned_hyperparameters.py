...
MODEL_PATH = r'Model_ENHANCMENT/contrastive_model_v3_best.pt'

LABELMAP_PATH = r'Model_ENHANCMENT/label_map_v3.pkl'

TEST_DATA_DIR = 'TEST_DATASET_small'

TEST_METADATA_CSV = 'TEST_DATASET_small/metadata.csv'

OUTPUT_DIR = 'FINAL_CNN_ANALYSIS_RESULTS'
...
def run_tsne_analysis(model, test_metadata, sr):
    ...
    if 'genre' not in test_metadata.columns:
        print("WARNING: genre column not found in metadata.")
        return
...
def main():
    ...
    try:
        ...
        print(f"ERROR: Label map file not found at {LABELMAP_PATH}. Please ensure this path is correct.")
        return

    print(f"Loading trained CNN model...")
    try:
        model = Encoder(emb_dim=256, n_classes=n_classes)
